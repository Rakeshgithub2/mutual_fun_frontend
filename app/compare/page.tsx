...
export default function ComparePage() {
        ...
        await Promise.all(
          selectedFunds.map(async (fund) => {
            try {
              ...
              console.error(`Failed to fetch details for ${fund.id}:`, error);
              // Use fallback data
              details[fund.id] = {
                ...fund,
                exitLoad: '1% (if redeemed within 1 year)',
  ...
  const canAnalyze = selectedFunds.length >= 2 && selectedFunds.length <= 4;

  // Analysis insights
  const insights = useMemo(() => {
    if (selectedFunds.length < 2) return null;

    const funds = selectedFunds.map((f) => detailedInfo[f.id] || f);
