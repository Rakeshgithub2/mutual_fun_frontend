...
export default function SearchPage() {
  ...
  const categories = [...new Set(mockFunds.funds.map((f) => f.category))]
  ...
  const results = useMemo(() => {
    return mockFunds.funds.filter((fund) => {
      ...
      const matchesCategory = !filters.category || fund.category === filters.category
      const matchesExpenseRatio =
        fund.expenseRatio >= filters.minExpenseRatio && fund.expenseRatio <= filters.maxExpenseRatio
      const matchesRating = fund.rating >= filters.minRating
      const matchesAUM = fund.aum >= filters.minAUM

      return matchesSearch && matchesCategory && matchesExpenseRatio && matchesRating && matchesAUM
    })
  }, [searchQuery, filters])
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-foreground">Search & Filter Funds</h1>
          <p className="mt-2 text-muted">Find the perfect fund for your investment goals</p>
        </div>
            ...
            <div className="rounded-lg border border-border bg-card p-6 sticky top-24">
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium text-foreground mb-2">
                  Expense Ratio: {filters.minExpenseRatio.toFixed(2)}% - {filters.maxExpenseRatio.toFixed(2)}%
                </label>
                <input
                  type="range"
                  min="0"
                  ...
                  onChange={(e) => setFilters({ ...filters, maxExpenseRatio: Number.parseFloat(e.target.value) })}
                  className="w-full"
                />
              </div>
              ...
              <div className="mb-6">
                ...
                <select
                  ...
                  onChange={(e) => setFilters({ ...filters, minRating: Number.parseInt(e.target.value) })}
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                >
              ...
              {/* AUM */}
              <div className="mb-6">
                <label className="block text-sm font-medium text-foreground mb-2">Minimum AUM (₹ Crore)</label>
                <input
                  type="number"
                  value={filters.minAUM}
                  onChange={(e) => setFilters({ ...filters, minAUM: Number.parseInt(e.target.value) || 0 })}
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                />
              </div>
          ...
          <div className="lg:col-span-3">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-foreground">
                Results: {results.length} fund{results.length !== 1 ? "s" : ""} found
              </h2>
            </div>
