...
export default function FundManagerPage({
  ...
  useEffect(() => {
    console.log("👤 Fund Manager Page - Current State:", {
      id,
      loading,
      error,
      hasFund: !!fund,
      ...
      managerExp: fund?.manager?.experience,
      managerQual: fund?.manager?.qualification,
    });
  }, [id, fund, loading, error]);
  ...
  const getManagerEducation = () => {
    ...
    const educationMap: { [key: string]: string } = {
      ...
      "MBA, CFA":
        "MBA in Finance, CFA (Chartered Financial Analyst) Charter Holder",
  ...
  // Management team - Use actual fund manager data from API
  const managementTeam = [
    {
      name: fund.manager?.name || "Senior Fund Manager",
      role: `Lead Fund Manager • ${fund.name}`,
      experience: fund.manager?.experience
        ? `${fund.manager.experience} years`
        : "15 years",
      expertise:
        fund.manager?.bio ||
        `Experienced fund manager specializing in ${fund.category} investments with a proven track record of delivering consistent returns.`,
      education:
        fund.manager?.qualification ||
        fund.manager?.qualifications?.[0] ||
        "MBA (Finance), CFA",
    },
    {
      name:
                    ...
                    <div className="p-4 bg-gradient-to-br from-purple-100 to-purple-200 dark:from-purple-900/40 dark:to-purple-800/40 rounded-xl border-2 border-purple-300 dark:border-purple-700">
                      ...
                      <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                        ...
                        fund.manager.experience >= 25
                          ? "6-8"
                          : fund.manager?.experience &&
                            fund.manager.experience >= 20
                          ? "4-6"
                          : fund.manager?.experience &&
                            fund.manager.experience >= 15
                          ? "2-4"
                          : "1-3"}
                      </p>
                    </div>
