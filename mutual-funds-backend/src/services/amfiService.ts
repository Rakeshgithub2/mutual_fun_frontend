...
export class AMFIService {
  ...
  private async processFundRecords(
    ...
    // Ensure fund exists - use findFirst + create instead of upsert to avoid MongoDB replica set requirement
    ...
    if (!fund) {
      ...
      // Update fund name if it changed
      fund = await prisma.fund.update({
        where: { id: fund.id },
        data: {
          name: firstRecord.schemeName,
          updatedAt: new Date(),
    ...
    // Batch process NAV records - use findFirst + create instead of upsert
    ...
    // Process NAV records - check existence first, then create or update
    for (const nav of navData) {
      const existingNav = await prisma.fundPerformance.findFirst({
        where: {
          fundId: nav.fundId,
          date: nav.date,
        },
      ...
      if (existingNav) {
        // Update existing NAV
        await prisma.fundPerformance.update({
