...
export function FundList({ funds, language }: FundListProps) {
  const [sortBy, setSortBy] = useState<"returns5Y" | "expenseRatio" | "rating">("returns5Y")
  const [filterCategory, setFilterCategory] = useState<string>("")
  const t = (key: string) => getTranslation(language, key)

  const categories = [...new Set(funds.map((f) => f.category))]

  const filtered = funds.filter((f) => !filterCategory || f.category === filterCategory)
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
              !filterCategory ? "bg-primary text-white" : "border border-border hover:bg-card"
            }`}
          >
            All
          </button>
          {categories.map((cat) => (
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
                filterCategory === cat ? "bg-primary text-white" : "border border-border hover:bg-card"
              }`}
            >
              {cat}
            </button>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {sorted.map((fund) => (
          <FundCard key={fund.id} {...fund} language={language} />
        ))}
      </div>
