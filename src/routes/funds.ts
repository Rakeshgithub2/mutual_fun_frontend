...
router.get('/:id/holdings', async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      data: { holdings: holdings.map(h => ({ ...h, _id: h._id.toString(), fundId: undefined })) },
...
router.get('/:id/details', async (req, res) => {
  try {
    ...
    const managers = await managersCollection.find({ fundId: new ObjectId(id) }).toArray();
    
    const details = {
      ...fund,
      id: fund._id.toString(),
      _id: undefined,
      fundManager: managers.length > 0 ? {
      ...
      assetAllocation: {
        ...
        debt: fund.category === 'debt' ? 95 : fund.category === 'equity' ? 2 : 30,
        cash: 3,
        others: 0
      }
    };
    ...
    return res.status(500).json({ success: false, error: 'Failed to fetch fund details' });
  }
});
